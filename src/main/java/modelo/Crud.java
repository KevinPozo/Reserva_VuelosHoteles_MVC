...
public class Crud {
	...
	public boolean crearUsuario(Usuario user) throws SQLException {
		...
		try {
			...
			sql = "INSERT INTO usuarios(nombre, apellido, correo_electronico, contrase̱a)"
					+ "VALUES(?,?,?,?)";
	...
	public boolean crearHabitacion(Habitacion habitacion) throws SQLException {
		...
		try {
			connection.setAutoCommit(false);
			sql = "INSERT INTO habitaciones(nombre_hotel, tipo_habitacion, precio_noche, disponibilidad)"
					+ "VALUES(?,?,?,?)";
			sentencia = connection.prepareStatement(sql);
	...
	public boolean crearVuelo(Vuelo vuelo) throws SQLException {
		...
		try {
			connection.setAutoCommit(false);
			sql = "INSERT INTO vuelos(origen, destino, fecha_salida, fecha_llegada, capacidad)"
					+ "VALUES(?,?,?,?,?)";
			sentencia = connection.prepareStatement(sql);
	...
	//-----------------------------------------------------
	//---------------HABITACION----------------------
	public Habitacion leerHabitacion(int id) throws SQLException {
		ResultSet resultados = null;
	...
	//-----------------------------------------------
	//---------------VUELO---------------------------
	public Vuelo leerVuelo(int id) throws SQLException {
		ResultSet resultados = null;
	...
	public boolean actualizarHabitacion(int id, Habitacion habitacion) throws SQLException {
		...
		try {
			connection.setAutoCommit(false);
			sql = "UPDATE habitaciones SET nombre_hotel=?, tipo_habitacion=?, precio_noche=?, disponibilidad=? WHERE id_habitacion =?";
			sentencia = connection.prepareStatement(sql);
	...
	public boolean actualizarVuelo(int id, Vuelo vuelo) throws SQLException {
		...
		try {
			connection.setAutoCommit(false);
			sql = "UPDATE vuelos SET origen=?, destino=?, fecha_salida=?, fecha_llegada=?, capacidad=? WHERE id_vuelo =?";
			sentencia = connection.prepareStatement(sql);
